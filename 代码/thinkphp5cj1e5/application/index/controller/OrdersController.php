...
class OrdersController extends CommonController
{
    ...
    public function page(){
        ...
        if ($data['isAdmin']!=1){
			$where['userid'] = $userid;//判断条件
                                                ...
                                                isset($_GET['type'])?($where['type'] = input('get.type')):$where;//订单查询
        $userid = Cache::get($token);
        if ($base['isAdmin']!=1){
            $where['userid'] = $userid;
        }
        ...
        $page_count = ceil($count/$limt);//页数
                ...
                $result = Db::table('orders')->where($where)->limit(($page-1)*$limt,$limt)->order($sort." ".$order)->select();
    ...
    public function update(){
        ...
        Db::table('orders')->where($where)->update($v);
        return json(['code'=>0]);
    }
    /**
     * 删除接口 post
    ...
    public function delete(){
        $ids = input('post.');
        $result = Db::table('orders')->delete($ids);
        return json(["code"=> 0]);
    }
    /**
     * 详情接口info ,后台接口
    ...
    public function info($id=false,$name=false){
        ...
        $result = Db::table('orders')->where($where)->find();
    ...
    public function remind($columnName,$type,$remindstart = false,$remindend = false){
        if ($type==1){
            ...
            $result = Db::table('orders')->where($map)->count();
        }else{
