...
class JiajufenleiController extends CommonController
{
    ...
    public function page(){
        ...
        $order = isset($_GET['order'])?input('get.order'):"asc";
                                        		                                                $count = Db::table('jiajufenlei')->where($where)->count();
        // 取整函数(ceil,floor,round)
        $page_count = ceil($count/$limt);//页数

                $result = Db::table('jiajufenlei')->where($where)->limit(($page-1)*$limt,$limt)->order($sort." ".$order)->select();
    ...
    public function save(){
		...
		$postData = $v;
                $result = Db::table('jiajufenlei')->insert($postData);
                return json(['code'=>0]);
    }
    /**
     * 保存接口 post
    ...
    public function update(){
        ...
        Db::table('jiajufenlei')->where($where)->update($v);
        return json(['code'=>0]);
    }
    /**
     * 删除接口 post
    ...
    public function delete(){
        $ids = input('post.');
        $result = Db::table('jiajufenlei')->delete($ids);
        return json(["code"=> 0]);
    }
    /**
     * 详情接口info ,后台接口
    ...
    public function info($id=false,$name=false){
        ...
        $result = Db::table('jiajufenlei')->where($where)->find();
    ...
    public function remind($columnName,$type,$remindstart = false,$remindend = false){
        if ($type==1){
            ...
            $result = Db::table('jiajufenlei')->where($map)->count();
        }else{
