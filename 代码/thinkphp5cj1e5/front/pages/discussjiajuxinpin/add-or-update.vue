	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjiajuxinpin`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussjiajuxinpin`, this.ruleForm);
				}else{
					await this.$api.add(`discussjiajuxinpin`, this.ruleForm);
				}
				uni.setStorageSync('discussjiajuxinpinCleanType',true);
